  ...
  export class OffreService {
    ...
    getAllOffresSorted(page = 0, size = 5, sortBy = 'dateCreation'): Observable<any> {
      const params = new HttpParams().set('page', page).set('size', size).set('sortBy', sortBy);

      return this.http.get<any>(`${this.baseUrl}/getAllSorted`, { params }).pipe(
        map((res) => ({
          ...res,
          content: (res.content ?? res).map(toFrontend),
        }))
    ...
    getOffreById(id: string): Observable<Offre> {
      return this.http.get<any>(`${this.baseUrl}/getById/${id}`).pipe(map(toFrontend));
    }

    updateOffre(id: string, offre: Offre): Observable<Offre> {
      return this.http
        .put<any>(`${this.baseUrl}/update/${id}`, toBackend(offre))
        .pipe(map(toFrontend));
    }
    ...
    affecterCriteresDeSelection(
      ...
      return this.http
        .put<any>(`${this.baseUrl}/affecterCriteresDeSelection/${idOffre}`, idCriteres)
        .pipe(map(toFrontend));
    }


    modifierStatusOffre(idOffre: string, status: string): Observable<any> {
      return this.http.put(
        `${this.baseUrl}/modifierStatusOffre/${idOffre}/${status}`,
        {}
      );
    }
